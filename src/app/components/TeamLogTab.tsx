...
const TeamLogTab: FC<TeamLogTabProps> = ({currentData, teamData, className, displayedLogs, setDisplayedLogs}) => {
    ...
    return (
        <div className={cn('group relative px-4 py-1 md:py-2 bg-slate-200 dark:bg-slate-600 flex items-center justify-between', className)}>
            <div className='flex'>
                <Paragraph size="xs" className={`group hover:line-through hover:cursor-pointer m-0 transition-colors ${isPresent ? "text-[#6c837d] dark:text-[#a3c8be]" : ""}`}>{teamData.team}</Paragraph>
