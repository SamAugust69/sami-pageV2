...
const EditableTextLabel: FC<EditableLabelProps> = ({
    label, onChange, placeholder, className, disabled
}) => {
    return (
        <Paragraph size="sm" className={cn(`m-0 text-left text-slate-500 flex`, className)}>
            {label}
            <span className='pl-1 m-0 text-left font-semibold text-slate-700 dark:text-slate-200'>
                ...
                <input disabled={disabled} className={"bg-transparent w-10 outline-none"} type="text" placeholder={placeholder === undefined || placeholder === "" || placeholder === "0" ? "N/A" : placeholder} onChange={onChange}/>
            </span>
        </Paragraph>
    )
}
